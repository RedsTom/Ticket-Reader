  ...
  <div class="message" :class="{[p.colorClass]: true}">
    ...
    <div class="right">
      ...
      <div class="content">
        ...
        <div class="embeds">
          <component :is="parser.comps.embed" :parser="parser" v-for="embed in p.message.embeds"
                     :key="JSON.stringify(embed)"
                     :embed="embed"/>
        </div>
        <div class="attachments" v-if="p.message.attachmentUrls.length > 0">
          <a
            class="attachment"
            v-for="att in Object.entries(p.message.attachmentUrls)"
